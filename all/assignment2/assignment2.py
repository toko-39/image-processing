...
def get_batch_image_vector(random_index, batch_image_number):
    #ベクトルを取得
    batch_images = test_images[random_index]
    return batch_images.reshape(batch_image_number, -1)

def get_batch_image_label(random_index, batch_image_number):
...
if __name__ == "__main__":
    ...
    one_hot_labels = get_one_hot_label(batch_labels, output_layer_size) # one-hot labelsを取得
    
    get_cross_entropy_error(output_probabilities, one_hot_labels) # クロスエントロピー誤差平均を計算
